...
### 4.2 推理速度（RTX 4090 D, FP16）
- YOLO11m@1280 单图推理（含 NMS）约 **8~12 ms**（batch=1），**理论 ~100+ FPS**，满足实时监考（≥25 FPS）需求。
- 全量 4443 张 B榜推理约 **50~60 秒**（单卡，无 batch 优化）。
- 自适应阈值推理仅增加 O(1) 的「按检出数选 conf」分支，**零额外耗时**。
- 部署建议：生产环境用 `batch=4~8` 的 TensorRT/ONNX 导出，单卡可支撑多路摄像头并发。
